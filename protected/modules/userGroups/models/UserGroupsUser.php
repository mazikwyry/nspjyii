...
class UserGroupsUser extends CActiveRecord
{
	...
	protected function afterSave()
	{
		...
		// send the needed emails for account approval
		if (($this->scenario === 'admin' || $this->scenario === 'registration') && $this->status === self::WAITING_APPROVAL) {
			$mail = new UGMail($this, UGMail::APPROVAL, TRUE);
			$mail->send();
		}
	...
	public function login($mode = 'regular')
	{
		...
		{
			if ($mode === 'regular') {
				$this->_identity=new UserGroupsIdentity($this->email,$this->password);
				$this->_identity->authenticate();
			} else if ($mode === 'recovery') {
				$this->_identity=new UserGroupsIdentity($this->email,$this->activation_code);
		...
		if($this->_identity->errorCode===UserGroupsIdentity::ERROR_NONE)
		{
			...
			$this->addError('password',Yii::t('userGroupsModule.recovery','wrong user or password.').'<br/>'.$this->_identity->errorCode.CHtml::link(Yii::t('userGroupsModule.recovery', 'Password Recovery'), array('/userGroups/user/passRequest')));
			return false;
	}
