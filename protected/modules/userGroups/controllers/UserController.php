...
class UserController extends Controller
{
	...
	public function actionRegister()
	{
		...
		if(isset($_POST['UserGroupsUser']))
		{
            ...
            $model->group_id = $_POST['add_to_group'];
			...
			if ($model->validate() && !isset($error)) {
				if($model->save()) {
                    ...
                    if(isset($_POST['service']))
                        $this->redirect(Yii::app()->baseUrl . '/userGroups/user/register?service='.$_POST['service']);
                    else
	...
	public function actionApprove()
	{
		if (isset($_POST['UserGroupsApprove'])) {
			...
			if ($model->save()){
				$mail = new UGMail($model, UGMail::APPROVED);
				$mail->send();
	...
	public function actionLogout()
	{
        ...
        if(Yii::app()->user->groupName=="hubert")
        {
    		Yii::app()->user->logout();
    		$this->redirect(Yii::app()->baseUrl . '/userGroups?service=herbert');
        }
        else {
            Yii::app()->user->logout();
            $this->redirect(Yii::app()->baseUrl. '/userGroups');
