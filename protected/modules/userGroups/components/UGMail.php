...
class UGMail {
	...
	public function __construct(UserGroupsUser $model, $message, $to_admin = FALSE)
	{
		...
		// extract user data
		$this->extractUserData($model, $to_admin);
		...
		if ($this->_swiftmail) {
			...
			if(!$to_admin)
				...
				$this->_message->addTo(Yii::app()->params->adminEmail, 'KP');
	 ...
	 */
	protected function extractUserData(UserGroupsUser $model, $to_admin)
	{	
		if(!$to_admin)
			$this->_to = "{$model->username} <{$model->email}>";
		else 
			$this->_to = "KP <".Yii::app()->params->adminEmail.">";
	...
	protected function defaultMailMessage($message)
	{
		switch ($message) {
			...
			case self::INVITATION:
				...
				break;
			case self::APPROVAL:
				return new UGMailApproval;
				break;
			case self::APPROVED:
				return new UGMailApproved;
				break;
		}
	}
...
/**
 * the mail message that will be sent to admin to approve
 * @author Nicola Puddu
 * @package userGroups
 */
class UGMailApproval implements UGMailMessage {
	...
	public function mailBody($data)
	{
		...
		return Yii::app()->controller->renderPartial('//ugmail/approval', array(
			'data' => $data
			), true);
	}
	...
	public function mailSuccess($data)
	{
		return Yii::t('userGroupsModule.general','The approval email was successfully sent to admin', $data);
	}
...
class UGMailApproved implements UGMailMessage {
	...
	public function mailSubject($data)
	{
		return 'Your accout has been aprroved!';
	}
	...
	public function mailBody($data)
	{
		...
		return Yii::app()->controller->renderPartial('//ugmail/approved', array(
			'data' => $data
			), true);
	}
	...
	public function mailSuccess($data)
	{
		return Yii::t('userGroupsModule.general','The approval email was successfully sent to user', $data);
	}
	...
	public function mailError($data)
	{
		return Yii::t('userGroupsModule.general','Impossible to send email to the address user', $data);
	}
}